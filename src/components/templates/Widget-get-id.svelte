<script>
	...
	const handleButtonClick = () => {
		const route = '/' + data.button.param.endpoint.split('.php')[0]
		const params = {
			action: data.button.param.action,
			data: data.button.param.data
		}
	...
	<div class="widget__wrapper">
		...
		{#if +data?.button?.isDisplayed}
			<div class="widget__button" class:disabled={isButtonDisabled}>
				<button type="button" on:click={handleButtonClick} disabled={isButtonDisabled}
