...
<aside class="aside">
	<div class="aside__inner">
		<div class="aside__accordion accordion">
			{#if $auth.isLoading}
				<div class="accordion__item" style="display: flex; flex-direction: column; gap: 30px;">
			...
			{:else}
				{#each Object.values(accordionItems) as items, idx}
					<div class="accordion__item" class:active={activeAccordionItem === idx}>
						<button on:click={() => toggleItem(idx)} class="accordion__item-trigger">
							...
							{#if items[0].icon && items.length > 1}
								<svg
									width="18"
									height="19"
									viewBox="0 0 18 19"
